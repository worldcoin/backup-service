//! Only ES256 / P-256 passkeys can be added as a factor: an existing passkey authorizes add-factor
//! through a Turnkey stamp, which is verified for ES256 only, so any other credential would be a
//! factor that can never authorize the next add-factor. The add-factor registration options
//! therefore advertise ES256 only (webauthn-rs's default list is `[ES256, RS256]`), but
//! `webauthn-rs` verifies an RS256 registration without complaint — so the server itself must
//! still turn one away when a client submits it anyway, with `unsupported_passkey_algorithm`,
//! and without consuming either challenge.
//!
...
/// Label sent with every new-factor authorization here.
...
impl ExistingOidcSession {
    ...
    /// the `existingFactorChallenge`, with no Turnkey provider id and no encrypted backup key.
...
/// The credential id (`rawId`) of a registration response: the mock authenticator emits a byte
/// array, real clients emit unpadded base64url.
fn raw_id_bytes(credential: &serde_json::Value) -> Vec<u8> {
    match &credential["rawId"] {
        serde_json::Value::Array(bytes) => bytes
            .iter()
            .map(|byte| u8::try_from(byte.as_u64().expect("byte")).expect("byte"))
            .collect(),
        serde_json::Value::String(encoded) => URL_SAFE_NO_PAD.decode(encoded).expect("base64url"),
        other => panic!("unexpected rawId: {other}"),
    }
}
...
async fn test_add_factor_rejects_rs256_passkey_registration_and_leaves_challenges_reusable() {
    ...
    let response = send_post_request_with_environment(
        "/v1/add-factor",
        session.add_factor_request(&challenges, &rsa_registration.credential),
    ...
    .await;
    let credential_id = raw_id_bytes(&credential);
    let response = send_post_request_with_environment(
        "/v1/add-factor",
        session.add_factor_request(&challenges, &credential),
        Some(backup.environment),
    )
    .await;
    let status = response.status();
...
async fn test_add_factor_accepts_hand_built_es256_passkey_registration() {
    ...
    let response = send_post_request_with_environment(
        "/v1/add-factor",
        session.add_factor_request(&challenges, &registration.credential),
        Some(backup.environment),
    )
    .await;
    let status = response.status();
