...
use crate::common::{
    add_factor_payload_for_oidc_new, create_test_backup_with_oidc_account, generate_keypair,
...
/// The `encryptedBackupKey` every request in this file sends; it is part of the bound material.
...
impl Ceremony {
    ...
    /// The bare `existingFactorChallenge` (standard base64), which is what a pre-binding client
    /// signed and what the existing factor must no longer sign on its own.
    fn bare_existing_challenge(&self) -> &str {
        self.challenges["existingFactorChallenge"].as_str().unwrap()
    }

    /// The existing factor's correct signature, over
    /// `existing_factor_challenge || SHA256(tag || new_factor_material)`.
    fn existing_signature(&self) -> String {
        ...
        sign_keypair_challenge(&self.existing.secret_key, &payload)
    }
...
async fn test_new_factor_signature_failure_consumes_nothing() {
    ...
    let wrong_new_signature =
        sign_keypair_challenge(&ceremony.new.secret_key, ceremony.bare_existing_challenge());
...
// 5. The existing factor signed the bare challenge instead of the material-bound payload: the
//    binding check rejects it after the new factor was already verified, and still nothing on
//    either side is consumed. The same tokens succeed with the right payload.
#[tokio::test]
#[serial]
async fn test_existing_factor_binding_failure_consumes_nothing() {
    let ceremony = Ceremony::prepare().await;
    let redis = get_test_redis_cache_manager().await;

    let legacy_existing_signature = sign_keypair_challenge(
        &ceremony.existing.secret_key,
        ceremony.bare_existing_challenge(),
    );
    let response = ceremony
        .send(ceremony.request_with(&legacy_existing_signature, &ceremony.new_signature()))
        .await;
    assert_rejected(
        response,
        StatusCode::BAD_REQUEST,
