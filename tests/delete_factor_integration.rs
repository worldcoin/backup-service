...
async fn test_delete_last_factor_happy_path() {
    ...
    assert!(response["backupMetadata"].is_null());
...
async fn test_delete_sync_factor_happy_path() {
    ...
    let create_response: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert!(
        !create_response["backupMetadata"]["id"].is_null(),
        "Failed to create backup received response: {create_response:?}"
    );
    let backup_id = create_response["backupMetadata"]["id"].as_str().unwrap();
...
async fn test_remove_sync_factor_etag_concurrency() {
    ...
    let sync_factor1 =
        backup_service::backup_metadata::Factor::new_ec_keypair("public-key-1".to_string());
    let sync_factor2 =
        backup_service::backup_metadata::Factor::new_ec_keypair("public-key-2".to_string());
    let sync_factor3 =
        backup_service::backup_metadata::Factor::new_ec_keypair("public-key-3".to_string());

    let initial_metadata = backup_service::backup_metadata::BackupMetadata {
        id: backup_id.clone(),
        factors: vec![],
        sync_factors: vec![
            sync_factor1.clone(),
            ...
            match err {
                backup_service::backup_storage::BackupManagerError::PutObjectError(sdk_err) => {
                    match sdk_err {
                        ...
                        _ => panic!(
                            "Expected ServiceError with PreconditionFailed, got: {sdk_err:?}"
                        ),
                    }
                }
