...
pub async fn handler(
    ...
    let (new_factor_type, new_factor_challenge_value, new_factor_token) = match &request.new_factor
    {
        NewFactor::PasskeyRegistration { platform } => {
            ...
            // Only ES256 / P-256 passkeys can be added as a factor (`/add-factor` rejects the rest
            // with `unsupported_passkey_algorithm`, see `webauthn::is_es256_p256`). Advertise only
            // what will be accepted, so no authenticator picks an algorithm the ceremony then fails
            // on. The registration state keeps webauthn-rs's default list; a credential outside the
